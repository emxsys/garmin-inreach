...
export default class App extends Component {
  ...
  componentDidMount() {
    let globe = this.globeRef.current

    // Important! Trigger a state change after the globe is mounted
    this.setState({globe: globe})
    
    // Get the user KML
    // TODO: obtain from https://inreach.garmin.com/feed/ShareLoader/<user>
    let kml = './feeds/feed-BruceSchubert.kml'
    let layer = globe.addLayer('renderables', {category: 'overlay', enabled: true, displayName: 'Bruce Schubert'})

    // Add the user to the layer
  ...
  render() {
    ...
    return (
      <div>
        <NavBar 
            logo=''
            title='Garmin inReach Tracker'
            href='https://github.com/emxsys/garmin-inreach'
            items={navbarItems}
            search={navbarSearch} />
